...
class time_deal():
    ...
    def get_and_dom(self):
        ...
        while not q1.empty():
            uid = q1.get()
            t = MyThread(func=self.w.get_user_weibo, args=(uid,))
            t.start()
            t.join()
        ...
        while not q2.empty():
            weibo = q2.get()
            t = MyThread(func=self.weibo_handle.dom_weibo, args=(weibo,))
            t.start()
            t.join()
    ...
    def forward(self):
        ...
        if not result:
            print("转发失败")
        self.weibo_handle.update_weibo(weibo_content)
    ...
    def make_weibo(self):
        ...
        if weibo_extend.get_weibo_pic():
            pic_id = self.w.upload_pic("pic/mie_word.jpg")
        self.w.original_weibo(content, pic_id)
    ...
    def food(self):
        ...
        pic_id = self.w.upload_pic("pic/food.jpg")
        self.w.original_weibo(content, pic_id)
...
def main():
    ...
    scheduler.add_job(t.food, 'cron', hour=22, minute=34) #美食推荐
    scheduler.start()
